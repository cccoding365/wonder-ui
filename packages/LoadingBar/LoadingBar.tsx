...
el.className = 'wonder_loadingBar-container'
if (!document.querySelector('.wonder_loadingBar-container')) {
...
function LoadingBarItem(props: LoadingBarProps): JSX.Element {
  ...
  return (
    <CSSTransition
      ...
      unmountOnExit
    >
      <div className="wonder_loadingBar_inner">
...
Ele.className = 'wonder_loadingBar_inner_box'
...
const RenderEle = (status: 'start' | 'finish' | 'error' | 'update', percents?: number, colors?: string, heights?: number) => {
  ...
  // 置入到指定节点下
  const container = document.querySelector('.wonder_loadingBar-container')
  if (container) {
