...
const Pagination: FC<PaginationProps> = ({
  ...
  return (
    <section
      className={`wonder-pagination-wrapper ${propClassName || ''}}`}
      style={{ ...propStyle }}
    ...
    >
      ...
      <ul className={'wonder-pagination'}>
        <li
          title={getDisplayText('prevPage')}
          className={classnames(
            'wonder-pagination-item',
            'wonder-pagination-item-prev',
            current === 1 ? 'wonder-pagination-disabled' : ''
          )}
          onClick={() =>
          ...
          pageNums.map((num) => {
            return (
              <li
                ...
                className={classnames(
                  'wonder-pagination-item',
                  current === num ? 'wonder-pagination-item-active' : ''
                )}
                tabIndex={num}
        ...
        <li
          ...
          className={classnames(
            'wonder-pagination-item',
            'wonder-pagination-item-next',
            current === lastPage ? 'wonder-pagination-disabled' : ''
          )}
          onClick={() =>
